...
public class FinancialResult {
	...
	// XPATH
	static String URL = "https://www.kotak.com/en/investor-relations/financial-results.html";
	...
	static ArrayList<String> QuarterAPIissue = new ArrayList<>();
	static ArrayList<String> issuequarter = new ArrayList<>();
	...
	static XSSFSheet Quarterapiissue = workbook.createSheet("QuarterAPIissues ");
	...
	public static void APIstatus(String nameofyear, String quarterName, int j) {
		...
		try {
...

			{
				...
				QuarterAPIissue.add(APIerror);
				issuequarter.add(quarterName); 
	...
	public static void storingDataintoExcel() throws IOException {
		...
		// Creating headers 
		Row pdferrorrow2 = Quarterapiissue.createRow(0);
		
		Cell cell10 = pdferrorrow2.createCell(0);
		cell10.setCellValue("YEAR");

		Cell cell11 = pdferrorrow2.createCell(1);
		cell11.setCellValue("QUARTER");

		Cell cell12 = pdferrorrow2.createCell(2);
		cell12.setCellValue("PDF TITLE");
		
		for (int p = 1; p < QuarterAPIissue.size(); p++) {
			Row row = Quarterapiissue.createRow(p);
			Cell cell4 = row.createCell(0);
			cell4.setCellValue(issueyear.get(p));

			Cell cell5 = row.createCell(1);
			cell5.setCellValue(issuequarter.get(p));

			Cell cell6 = row.createCell(2);
			cell6.setCellValue(QuarterAPIissue.get(p));
		}
		
		
		File f = new File("D:\\backup\\Projects\\Intranet\\src\\test\\java\\resultinExcel\\FinanciaResult.xls");
